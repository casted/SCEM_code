...
class load_data(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data_lowlight = np.asarray(data_low).transpose(1, 2, 0)
        light, ref, sha = get_lrs(data_lowlight)
        light = torch.Tensor(light).float().permute(2, 0, 1)
        ref = torch.Tensor(ref).float().permute(2, 0, 1)
        sha = torch.Tensor(sha).float().permute(2, 0, 1)
        ...
        return [data_low, data_high,data_color,data_blur, light, ref, sha]
...
class load_data_test(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return [data_low, data_high,data_color,data_blur,self.input_data_low[idx], light, ref, sha]
...
def sort_key(path):
    ...
    m = re.match(r'(.*)_(?:low|high)_(.*)', base)
    if m:
...
def train(config: Dict):
    ...
    datapath_train_low = sorted(glob.glob(train_low_path), key=sort_key)
    datapath_train_high = sorted(glob.glob(train_high_path), key=sort_key)
    ...
    if config.pretrained_path is not None:
        ...
        net_model.load_state_dict({k.replace('module.', ''): v for k, v in ckpt.items()}, strict=False)
    ...
    for e in range(config.epoch):
        ...
        with tqdm(dataloader, dynamic_ncols=True) as tqdmDataLoader:
            for data_low, data_high, data_color, data_blur, light, ref, sha in tqdmDataLoader:
                ...
                ref = ref.to(device)
                sha = sha.to(device)
                data_concate=torch.cat([data_color, snr_map, light, ref, sha], dim=1)
                optimizer.zero_grad()
...
def Test(config: Dict,epoch):
    ...
    datapath_test_low = sorted(glob.glob(test_low_path), key=sort_key)
    datapath_test_high = sorted(glob.glob(test_high_path), key=sort_key)
    ...
    with torch.no_grad():
        with tqdm( dataloader, dynamic_ncols=True) as tqdmDataLoader:
                image_num = 0
                for data_low, data_high, data_color,data_blur,filename,light, ref, sha in tqdmDataLoader:
                    ...
                    brightness_level=gt_image.mean([1, 2, 3])
...
if __name__== "__main__" :
    ...
    parser.add_argument('--dataset_path', type=str, default="../LOLDataset/")
    parser.add_argument('--state', type=str, default="train")
    parser.add_argument('--pretrained_path', type=str, default="./output_try/ckpt_14200_.pt")
    parser.add_argument('--output_path', type=str, default="./output_try/")
